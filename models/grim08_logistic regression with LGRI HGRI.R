...
multinom_retinopathy = model_df %>% 
  ...
  nnet::multinom(d_retinopathy ~ HGRI10 + LGRI10 + ageonset + d_duration + gender + hba + bmi,
                 data=.)
...
multinom_nephropathy = model_df %>% 
  dplyr::filter(data_partition == "Training") %>% 
  nnet::multinom(d_nephropathy ~ HGRI10 + LGRI10 + ageonset + d_duration + gender+ hba + bmi,
                 data=.)

# Prevalent DKD ~ GRI ---------------------
glm_dkd = model_df %>% 
  dplyr::filter(data_partition == "Training") %>% 
  glm(dkd ~ HGRI10 + LGRI10 + ageonset + d_duration + gender+ hba + bmi,
...
summary_df = bind_rows(broom::tidy(multinom_nephropathy) %>% 
  ...
  dplyr::filter(str_detect(term,"(H|L)GRI10")) 

saveRDS(summary_df,paste0(path_gri_working,"/logistic regression with LGRI HGRI.RDS"))
