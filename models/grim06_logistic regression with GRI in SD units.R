...
multinom_nephropathy = model_df %>% 
  dplyr::filter(data_partition == "Training") %>% 
  nnet::multinom(d_nephropathy ~ GRISD + ageonset + d_duration + gender+ hba + bmi,
                 data=.)

# Prevalent DKD ~ GRI ---------------------
glm_dkd = model_df %>% 
  dplyr::filter(data_partition == "Training") %>% 
  glm(dkd ~ GRISD + ageonset + d_duration + gender+ hba + bmi,
...
summary_df = bind_rows(broom::tidy(multinom_nephropathy) %>% 
  ...
  dplyr::filter(str_detect(term,"GRISD")) 

saveRDS(summary_df,paste0(path_gri_working,"/logistic regression with GRI SD.RDS"))
