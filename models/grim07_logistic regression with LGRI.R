...
model_df <- gri_df %>% 
  ...
  mutate_at(vars(d_retinopathy_yn,
            ...
            function(x) case_when( x  == "0, No" ~ 0,
                                   x == "1, Yes" ~ 1,
                                   TRUE ~ NA_real_)) %>% 
  mutate(LGRI10 = LGRI/10) 

# Prevalent Retinopathy ~ GRI ----------------------

multinom_retinopathy = model_df %>% 
  dplyr::filter(data_partition == "Training") %>% 
  nnet::multinom(d_retinopathy ~ LGRI10 + ageonset + d_duration + gender + hba + bmi,
                 data=.)
...
multinom_nephropathy = model_df %>% 
  dplyr::filter(data_partition == "Training") %>% 
  nnet::multinom(d_nephropathy ~ LGRI10 + ageonset + d_duration + gender+ hba + bmi,
                 data=.)

# Prevalent DKD ~ GRI ---------------------
glm_dkd = model_df %>% 
  dplyr::filter(data_partition == "Training") %>% 
  glm(dkd ~ LGRI10 + ageonset + d_duration + gender+ hba + bmi,
...
summary_df = bind_rows(broom::tidy(multinom_nephropathy) %>% 
  ...
  dplyr::filter(str_detect(term,"LGRI10")) 

saveRDS(summary_df,paste0(path_gri_working,"/logistic regression with LGRI.RDS"))
